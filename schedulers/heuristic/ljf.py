    ...
    def schedule(self, obs):
        ...
        if source_job_idx < num_active_jobs:
            ...
            if local_job.frontier_stages:
                ...
                if num_exec > 0:
                    selected_stage = max(local_job.frontier_stages, key=lambda s: s.remaining_work)
                    return {
        ...
        selected_job = max(candidate_jobs, key=lambda job: job.total_remaining_work)
        selected_stage = max(selected_job.frontier_stages, key=lambda s: s.remaining_work)
