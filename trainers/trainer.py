...
class Trainer(ABC):
    ...
    def train(self):
        ...
        for i in range(self.num_iterations):
            ...
            avg_num_queue = np.mean([stats["avg_queue_length"] for stats in rollout_stats_list])
            avg_job_dur = np.mean([stats["avg_job_duration"] for stats in rollout_stats_list])
            ...
            print(
                ...
                f"Training time: " f"{curr_comp_time-past_comp_time:.0f}. "
                f"Avg. training time: " f"{sum(training_time)/len(training_time):.1f}. "
                f"Avg. # jobs: " f"{avg_num_jobs:.3f}, Avg. # queue: " f"{avg_num_queue:.1f},"
