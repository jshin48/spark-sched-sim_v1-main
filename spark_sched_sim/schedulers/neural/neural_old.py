...
class NeuralScheduler(Scheduler):
    ...
    def schedule(self, obs: ObsType) -> ActType:
        ...
        if self.name == "HyperHeuristic":
            ...
            heuristic_score = self.actor.priority_heuristic_policy_network(h_dict)
            heuristic_idx, lgprob = self._sample(heuristic_score)

            if heuristic_idx == 0:
                scheduler = McScheduler(self.num_executors, dynamic_partition=True)
            elif heuristic_idx == 1:
                scheduler = WscptScheduler(self.num_executors, dynamic_partition=True)
            else:
        ...
        if self.resource_allocation == 'Random':
            ...
            resource_heuristic_score = self.actor.resource_heuristic_policy_network(h_dict)
    ...
    def evaluate_actions(self, dag_batch, actions):
        ...
        #JS Revision
        heuristic_selections, stage_selections, job_indices, exec_selections = \
        ...
        if self.name == "HyperHeuristic":
            ...
            action_lgprobs = heuristic_lgprobs + exec_lgprobs
            action_entropies = heuristic_entropies + exec_entropies
            print("Need to Check again the following line")
