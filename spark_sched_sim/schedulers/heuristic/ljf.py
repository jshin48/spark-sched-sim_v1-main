...
class LjfScheduler(HeuristicScheduler):
        ...
        if schedulable_stages:
            ...
            if selected_job_idx == -1:
                ...
                for job in range(num_active_jobs):
                    selected_job_idx = max(job_cpt,key = job_cpt.get)
                ...
                if num_exec == -1 and self.resource_allocation == 'DRA':
                    print("num_exec is negative", obs["DRA_exec_cap"][selected_job_idx], obs["exec_supplies"][selected_job_idx],
                               obs["num_committable_execs"],job_cpt)
