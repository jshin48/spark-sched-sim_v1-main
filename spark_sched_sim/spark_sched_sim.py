...
class SparkSchedSimEnv(Env):
    ...
    def __init__(self, env_cfg, agent_cfg= {"agent_cls": "Heuristic"} ):
        ...
        if self.render_mode == "human":
            ...
            self.renderer = Renderer(
                ...
                plot_title= env_cfg["plot_title"],
                render_fps=self.metadata["render_fps"],
            )

        self.job_duration_buff: deque[float] = deque(maxlen=200)

        if self.agent_cls == 'DecimaScheduler':
        ...
        self.observation_space = sp.Dict(
            {
                ...
                # edge features: none
                "dag_batch": sp.Graph(
                    node_space=sp.Box(0, np.inf, (self.NUM_NODE_FEATURES,)),
                ...
                #JS added
                "DRA_exec_cap":  sp.Sequence(sp.Discrete(self.num_executors + 1), stack=True),
            }
        )
    ...
    def step(self, action: dict) -> tuple[dict, float, bool, bool, dict]:
        ...
        self.queue_length.append(len(self.schedulable_stages))
    ...
    def get_avg_task_duration(self):
        if self.total_num_completed_task > 0:
            avg_task_duration = self.total_completed_task_duration/self.total_num_completed_task
        else:
            avg_task_duration = self.total_completed_task_duration
    ...
    def _observe(self) -> dict[str, Any]:
        ...
        edges = np.zeros(len(edge_links), dtype=np.int64)
    ...
    def _handle_task_completion(self, stage: Stage, task: Task) -> None:
        ...
        self.total_num_completed_task += 1
        self.total_completed_task_duration += task.t_completed - task.t_accepted
